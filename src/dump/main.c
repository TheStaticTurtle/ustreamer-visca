...
static int _dump_sink(
	...
	while (!_g_stop) {
		...
		if (got == 0) {
			...
			char fourcc_str[8];
			US_LOG_DEBUG("Frame: %s - %ux%u -- online=%d, key=%d, kr=%d, gop=%u, latency=%.3Lf, backlog=%.3Lf, size=%zu",
			...
			US_LOG_TRACE("       stride=%u, grab_ts=%.3Lf, encode_begin_ts=%.3Lf, encode_end_ts=%.3Lf",
